...
**Implementation:** `src/sensor_app/api/main.py` · **LLM:** `src/sensor_app/llm/` · **Stream consumer:** `src/sensor_app/lib/stream_consumer.py` · **Specs:** `spec_ch1.md`, `spec_ch2.md`, `spec_ch3.md`.
...
**Purpose:** List stored metric snapshots for the station. Optional query params: `start_time`, `end_time`, `device_id` (filters devices in each snapshot’s JSON), **`metric_key`** (keep only that scalar per device in the response).
...
curl -s "http://127.0.0.1:8000/metrics/STATION_ID?metric_key=uptime_seconds"
```
...
**Purpose:** Counters and backlog when a stream consumer is running (`SENSOR_APP_STREAM_CONSUMER_ENABLED` + **`event_queue`** on **`create_app`**, or **`run_streaming_stack.py`**). If disabled, returns **`enabled: false`** and a short **`detail`**.

**Example:**

```bash
curl -s "http://127.0.0.1:8000/stream/status"
```
...
**Purpose:** Flush the in-memory **`StreamProcessor`** buffer through the pipeline into **`metric_snapshots`**. Does **not** drain the queue. Returns **503** if no consumer is configured.

**Example:**

```bash
curl -s -X POST "http://127.0.0.1:8000/stream/flush"
```
...
- **Stream flush with no consumer:** **`503`** on **`POST /stream/flush`**.
- **Env:** root **`README.md`** configuration table; streaming runbook under **Streaming stack (spec_ch3)**.
